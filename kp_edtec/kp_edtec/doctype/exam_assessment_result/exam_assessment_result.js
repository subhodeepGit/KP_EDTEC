...
frappe.ui.form.on('Exam Assessment Result', {
	refresh: function(frm) {
		...
		frm.set_query('course','assessment_result_item', function() {
			return {
				query: 'kp_edtec.kp_edtec.doctype.exam_assessment_result.exam_assessment_result.filter_courses',
				filters: {
	...
	setup:function(frm){
		...
		// 	return {
		// 		query: 'kp_edtec.kp_edtec.doctype.student_group.filter_student',
		// 		filters: {
    ...
    student:function(frm){
    	if(frm.doc.student){
	    	frappe.call({
				method: "kp_edtec.kp_edtec.doctype.exam_assessment_result.exam_assessment_result.get_student_details",
				args: {
					student:frm.doc.student,
				},
				callback: function(r) { 
    	...
    	if(frm.doc.student && frm.doc.academic_year && frm.doc.academic_term){
			frappe.call({
				...
				callback: function() {
	                frm.refresh();
	                frappe.call({
						method: "kp_edtec.kp_edtec.doctype.exam_assessment_result.exam_assessment_result.get_assessment_status",
						args: {
							student:frm.doc.student,
