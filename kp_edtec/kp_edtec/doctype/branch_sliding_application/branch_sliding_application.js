...
frappe.ui.form.on('Branch Sliding Application', {
		...
		if (frappe.user.has_role(["Student","Instructor"]) && !frappe.user.has_role('System Manager')){
			...
			frm.add_custom_button(__("Enroll"), function() {
				frappe.model.open_mapped_doc({
					method: "kp_edtec.kp_edtec.doctype.branch_sliding_application.branch_sliding_application.enroll_student",
					frm: frm
				})
	...
	setup(frm){
		frm.set_query("sliding_in_program", function() {
			return {
				query: 'kp_edtec.kp_edtec.doctype.branch_sliding_application.branch_sliding_application.get_sliding_in_progam',
				filters: {
	...
	student(frm){
		if (frm.doc.student){
			frappe.call({
				method: "kp_edtec.kp_edtec.doctype.branch_sliding_application.branch_sliding_application.get_student_details",
				args: {
	...
	sliding_in_program(frm){
		...
		if (frm.doc.branch_sliding_declaration && frm.doc.sliding_in_program){
			frappe.call({
				method: "kp_edtec.kp_edtec.doctype.branch_sliding_application.branch_sliding_application.get_declaration_details",
				args: {
