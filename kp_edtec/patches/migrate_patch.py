...
#   bench execute kp_edtec.patches.migrate_patch.get_custom_role_permission
...
#   bench execute kp_edtec.patches.migrate_patch.set_custom_role_permission
def set_custom_role_permission():
    with open(frappe.get_app_path("kp_edtec","fixtures","custom_docperm.json")) as f:
...
#   bench execute kp_edtec.patches.migrate_patch.set_custom_role_permission
def set_translation():
    with open(frappe.get_app_path("kp_edtec","fixtures","translation.json")) as f:
        for d in json.load(f):
...
#   bench execute kp_edtec.patches.migrate_patch.add_roles
def add_roles():
    with open(frappe.get_app_path("kp_edtec","fixtures","role.json")) as f:
        for d in json.load(f):
...
def add_module_profile():
    with open(frappe.get_app_path("kp_edtec","fixtures","module_profile.json")) as f:
        for d in json.load(f):
