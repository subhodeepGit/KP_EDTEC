...
frappe.ui.form.on('Exam Assessment Plan', {
	setup:function(frm) {
		...
		frm.set_query('program',function() {
		    return {
				query: 'kp_edtec.ed_tec.doctype.exam_assessment_plan.exam_assessment_plan.get_sem',
				filters: {
		...
		frm.set_query('paper_setter','examiners_list', function(frm, cdt, cdn) {
		    ...
		    return {
		    	query: 'kp_edtec.ed_tec.doctype.exam_assessment_plan.exam_assessment_plan.filter_paper_setter',
				filters: {
					"course":d.course
				}
			};
		...
		frm.set_query('moderator','moderator_list', function() {
			...
			frm.set_query("course","course_assessment_plan_item",function() {
				return {
					query: 'kp_edtec.ed_tec.doctype.exam_assessment_plan.exam_assessment_plan.get_courses',
					filters: {
	...
	refresh: function(frm) {
		...
		// 	// 		method: "ed_tec.ed_tec.doctype.student_group.create_student_group",
		// 	// 		frm: frm,
		// 	// 	});
		// 	// }, __('Create'))
	...
	student_group:function(frm){
		if (frm.doc.student_group) {
			frappe.call({
				method: 'kp_edtec.ed_tec.doctype.exam_assessment_plan.exam_assessment_plan.course_assessment_credit',
				args: {
					student_group: frm.doc.student_group
				},
				callback: function(r) {
		            ...
		            $.each(tabletransfer.courses_offered, function(index, row){
						if (row.semester==frm.doc.program){
							...
							if (d.course && frm.doc.assessment_criteria){
								frappe.call({
									method: 'kp_edtec.ed_tec.doctype.exam_assessment_plan.exam_assessment_plan.get_assessment_criteria_detail',
									args: {
										course:d.course,
