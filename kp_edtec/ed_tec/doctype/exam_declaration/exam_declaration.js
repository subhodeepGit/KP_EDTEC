...
frappe.ui.form.on('Exam Declaration', {
    refresh: function(frm){
        ...
        //             // /opt/bench/frappe-bench/apps/ed_tec/ed_tec/ed_tec/doctype/exam_declaration/exam_declaration.py
		// 			doc: frm.doc,
		// 			callback: function() {
		// 				frm.refresh();
		// 			}
  ...
  //                   "docstatus":1
        ...
        frm.fields_dict['courses_offered'].grid.get_field("courses").get_query = function(doc, cdt, cdn) {
            var semesters = cur_frm.doc.semesters.map(d => d.semester);
            return {
                query: 'kp_edtec.ed_tec.doctype.exam_declaration.exam_declaration.filter_courses',
                filters: {
                    "program":semesters
                }
            };
    ...
    get_students:function(frm){
		...
		frappe.call({
			method: "kp_edtec.ed_tec.doctype.exam_declaration.exam_declaration.get_students",
			args: {
				programs: frm.doc.exam_program,
