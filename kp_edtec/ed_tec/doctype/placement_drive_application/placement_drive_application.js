...
frappe.ui.form.on('Placement Drive Application', {
	...
	setup:function(frm){
		frm.set_query("placement_drive", function() {
			return {
				query:"kp_edtec.ed_tec.doctype.placement_drive_application.placement_drive_application.get_placement_drive",
				filters: {
					"docstatus": 1,
	...
	status:function(frm){
		if(frm.doc.status){
			frappe.call({
				method: "kp_edtec.ed_tec.notification.custom_notification.placement_drive_application_submit",
                args:{
                	'doc':frm.doc
                },
				callback: function(r) { 
