...
frappe.ui.form.on('Counselling Structure', {
	refresh: function(frm) {
			...
			// 		method: "ed_tec.ed_tec.doctype.counselling_structure.counselling_structure.create_student_admission",
			// 		frm: frm,
			// 	});
			// }, __('Create'))
	...
	setup: function(frm) {
		...
		frm.fields_dict['counselling_programs'].grid.get_field('programs').get_query = function(doc, cdt, cdn) {
            return {   
                query: 'kp_edtec.ed_tec.doctype.counselling_structure.counselling_structure.filter_programs_by_department', 
                filters:{
