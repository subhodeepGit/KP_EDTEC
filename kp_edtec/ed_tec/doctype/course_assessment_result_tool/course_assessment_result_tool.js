...
frappe.ui.form.on('Course Assessment Result Tool', {
	setup:function(frm){
        ...
        frm.set_query("course", function() {
			return {
				query:"kp_edtec.ed_tec.doctype.course_assessment_result_tool.course_assessment_result_tool.get_courses",
				filters: {
        ...
        frm.set_query("assessment_criteria", function() {
			return {
				query:"kp_edtec.ed_tec.doctype.course_assessment_result_tool.course_assessment_result_tool.get_assessment_criteria_list",
				filters: {
	...
	assessment_criteria: function(frm) {
		...
		frm.page.set_primary_action(("Submit"), function() {
			...
			frappe.call({
				method: "kp_edtec.ed_tec.doctype.course_assessment_result_tool.course_assessment_result_tool.make_course_assessment",
				args: {
	...
	get_student_details:function(frm){
		if(frm.doc.academic_year && frm.doc.academic_term && frm.doc.course && frm.doc.assessment_criteria && frm.doc.programs && frm.doc.semester) {
			frappe.call({
				method: "kp_edtec.ed_tec.doctype.course_assessment_result_tool.course_assessment_result_tool.get_enroll_students",
				args: {
					"academic_year":frm.doc.academic_year,
					"academic_term":frm.doc.academic_term,
					"programs":frm.doc.programs,
